...
func TestNewSpecialPower(t *testing.T) {
	...
	assert.Equal(t, cards.SpecialPowerMaxHealth, sp.Health())
	assert.Equal(t, cards.SpecialPowerDamage, sp.DamageAmount())
...
func TestSpecialPower_Destroyed(t *testing.T) {
	...
	sp := &cards.SpecialPower{
		cardBase: &cards.CardBase{
			cardMovedToPileObserver: discardObs,
		},
		attackableBase: &cards.AttackableBase{
			attackedBy: []cards.Weapon{weapon1, weapon2},
		},
	}
